...
let customtags = [
    ...
    // ["futurepack:dust_obsidian", "mekanism:dust_obsidian"]
]
...
if (Platform.isForge()) {
    ...
    let tagGen = [
        ...
        "osmium=ingots,ores",
        "sulfur=dusts,ores",
        "netherite,obsidian,charcoal,coal,emerald,iron,lapis=dusts",
        "silicon=gems"
    ]
        ...
        try {
            for (let tag of global["unifytags"]) {
                ...
                if (ingr) {
                    ...
                    let tItem = global["tagitems"][tag]
                    for (let s of stacks) {
                        if (s.getId() != tItem && !global["unifyexclude"].has(s.getId())) console.log(s.getId());
                    }
                }
            }
        } catch (err) {
...
const conflictRemovalID1 = [
    ];

const conflictRemovalOutput1 = [
];

const conflictRemovalInput1 = [
    "tinkers_reforged:aluminum_ore",
    "tinkers_reforged:raw_aluminum"
];
onEvent('recipes', event => {

    conflictRemovalInput1.forEach(item2 => {
      event.remove({input: item2})
    });
